  ...
  useEffect(() => {
    async function getLComComparaDia() {
      await birel
        ...
        .then(results => {
          setLComComparaDia(results.data.bi001.bidata);
        })
        .catch(err => {
          console.log(err);
        });
      ...
      <BTable classname="text-gray-50">
        <thead>
          <BTr classname="">
            ...
            <BTh classname="w-16">Compra dia {lComTotais.map((d: any) => d.DiaAtual)}</BTh>
            <BTh classname="w-16">Compra dia {lComTotais.map((d: any) => d.DiaAnterior)}</BTh>

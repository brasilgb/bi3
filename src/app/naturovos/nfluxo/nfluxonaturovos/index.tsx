...
const NFluxoNaturovos = (props: Props) => {
  ...
  return (
    <div className="w-full bg-solar-orange-prymary text-gray-800 rounded-t-md shadow-sm overflow-auto animate__animated animate__fadeIn">
      ...
      <BTable>
      <BTr classname="flex justify-between text-sm  bg-gray-800 text-gray-50">
          <BTd classname="flex items-start">
            <div className="w-[206px] ml-1 pl-4 uppercase">Descrição</div>
          </BTd>
            <BTd classname="flex justify-start">
              <div className=" text-left">{moment(dataInicial).format("DD/MM/YYYY")} - {moment(dataFinal).format("DD/MM/YYYY")}</div>
            </BTd>
        </BTr>
          ...
          .map((fluxo1: any, idx: number) => (
            <>
              <BTr
                ...
                key={idx}
                classname={`flex justify-between text-sm !border-b-gray-100 text-gray-500 bg-gray-200 transition-colors duration-150 hover:bg-solar-orange-prymary/10 ${caretLevel(2, fluxo1.codigo) ? 'cursor-pointer' : 'cursor-default'}`}
              >
                      ...
                      .map((fluxo2: any, idx: number) => (
                        <>
                          <BTr
                            ...
                            key={idx}
                            classname={`flex justify-between !border-b-gray-50 bg-gray-100 text-gray-500 transition-colors duration-150 hover:bg-solar-orange-prymary/10 ${caretLevel(2, fluxo1.codigo) ? 'cursor-pointer' : 'cursor-default'}`}
                          >
                            <BTd classname="flex items-center">
                              ...
                              <BTd colspan={2} classname="!p-0">
                                {fluxoDataParcialLojas
                                  ...
                                  .map((fluxo3: any, idx: number) => (
                                    <BTr
                                      key={idx}
                                      classname={`flex justify-between bg-gray-50 text-gray-500 transition-colors duration-150 hover:bg-solar-orange-prymary/10`}
                                    >
                                      <BTd classname="flex items-center">
...
export default NFluxoNaturovos;

...
const NFluxoGrupoData = (props: Props) => {
  ...
  const descricao2 = fluxoDataParcialLojas.filter(
    (ds: any) =>
      ds.nivel === 2 &&
      ds.coluna === 1 &&
      removeAcentos(ds.agrupadorNome) === levelValue
  );
  const descricao3 = fluxoDataParcialLojas.filter(
    (ds: any) =>
      ds.nivel === 3 &&
      ds.coluna === 1 &&
  ...
  const caretLevel = (nivel: number, codigo: any) => {
    return fluxoDataParcialLojas.filter(
      (lv: any) =>
        lv.nivel === nivel && codigo == removeAcentos(lv.agrupadorNome)
    ).length;
  };
  const uniqueData = Array.from(
    new Set(
      fluxoDataParcialLojas
        .filter((f: any) => f.nivel === 1 && f.data != null)
        .map((item: any) => item.data)
    )
  );

  return (
    <div className="w-full bg-solar-orange-prymary text-gray-800 rounded-t-md shadow-sm overflow-x-auto animate__animated animate__fadeIn">
      ...
      <BTable>
        <BTr classname="flex justify-start text-sm  bg-gray-800 text-gray-50">
        ...
        {uniqueDescricao1.map((fluxo1: any, idx: number) => (
          <>
            <BTr
              ...
              key={idx}
              classname={`flex justify-start text-sm !border-b-gray-100 text-gray-500 bg-gray-200 transition-colors duration-150 hover:bg-solar-orange-prymary/10 ${caretLevel(2, removeAcentos(fluxo1)) ? 'cursor-pointer' : 'cursor-default'}`}
            >
              <BTd classname="flex items-center">
                {caretLevel(2, removeAcentos(fluxo1)) ? (
                  <span
                    className={`text-gray-500 duration-300 ${levelOpen && levelValue === removeAcentos(fluxo1) ? '-rotate-180' : 'rotate-0'}`}
                  >
                ...
                <span className="text-sm text-gray-500 w-48">{fluxo1}</span>
              </BTd>
              {dataNivel1
                .filter(
                  (n1: any) =>
                  ...
                  {uniqueDescricao2.map((fluxo2: any, idx: number) => (
                    <>
                      <BTr
                        ...
                        key={idx}
                        classname={`flex justify-start !border-b-gray-50 bg-gray-100 text-gray-500 transition-colors duration-150 hover:bg-solar-orange-prymary/10 ${caretLevel(3, removeAcentos(fluxo2)) ? 'cursor-pointer' : 'cursor-default'}`}
                      >
                        <BTd classname="flex items-center">
                          {caretLevel(3, removeAcentos(fluxo2)) ? (
                            <span
                              className={`ml-2 text-gray-500 duration-300 ${levelOpen2 && levelValue2 === removeAcentos(fluxo2) ? '-rotate-180' : 'rotate-0'}`}
                            >
                              <IoChevronDown size={18} />
                            </span>
                          ) : (
                          ...
                          <span className="text-gray-500 w-[184px]">
                            {fluxo2}
                          </span>
                        </BTd>
                        {dataNivel2
                      ...
                      {levelOpen2 && levelValue2 === removeAcentos(fluxo2) && (
                        <BTr classname="!p-0 flex flex-col">
                          <BTd colspan={2} classname="!p-0">
                            {uniqueDescricao3.map(
                              (fluxo3: any, idx: number) => (
                                <BTr
                                  key={idx}
                                  classname={`flex justify-start bg-gray-50 text-gray-500 transition-colors duration-150 hover:bg-solar-orange-prymary/10`}
                                >
                                  <BTd classname="flex items-center">
                                    <span className="ml-4 text-gray-400">
                                      <IoRemove size={14} />
                                    </span>
                                    <span className="pl-1 w-[181px]">
                                      {fluxo3}
                                    </span>
                                  </BTd>
                                  {dataNivel3
...
export default NFluxoGrupoData;
